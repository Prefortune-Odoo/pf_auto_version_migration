...
class MigrationWizard(models.TransientModel):
    ...
    def _get_custom_app_list(self):
        ...
        list = self.env['ir.module.module'].search([]).filtered(lambda x : x.author not in ['Camptocamp / Odoo','Odoo', 'Odoo S.A.','Eezee-It','l10','Odoo SA','Camptocamp','Odoo S.A, Applix BV, Droggol Infotech Pvt. Ltd.'] and 'l10' not in x.name and x.name not in ['pf_auto_version_migration', 'report_xlsx'])
        return list
    ...
    def _get_ip_address(self):
        try:
            ...
            return "Could not get public IP: {}".format(e)
    ...
    def _get_destination_versions(self):
        current_version = int(float(release.series))
        versions = []
        for v in range(current_version + 1, 20):
            versions.append(("{}.0".format(v), "{}.0".format(v)))
        return versions
    ...
    #     selection=_get_destination_versions,
    #     string='Target Version',
    #     default=lambda self: self._get_destination_versions()[-1][0] if self._get_destination_versions() else False,
    #     required=True
    # )
    ...
    def action_next_step(self):
        ...
        if self.active_state == 'first':
            if not self.destination_version:
                raise ValidationError("Please Select Destination Version !!! ")
    ...
    def action_create_ticket(self):
        ...
        url = base_url + '/migration-server/create/ticket'
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                migration_ticket = self.env['pf.migration.ticket'].create(create)
                ...
                if status == 'success':
                    ...
                    raise UserError("Failed to create ticket: {}".format(error_msg))
            else:
                raise UserError(
                        "Failed to create ticket. Status: {}, Response: {}".format(
                            response.status_code, response.text
                        )
            )
        except requests.exceptions.RequestException as e:
            _logger.error("Error calling migration server: %s", str(e))
            raise UserError("Error connecting to migration server: {}".format(str(e)))
