...
class MigrationWizard(models.TransientModel):
    ...
    def _get_custom_app_list(self):
        ...
        list = self.env['ir.module.module'].search([]).filtered(lambda x : x.author not in ['Camptocamp / Odoo','Eezee-It','Odoo', 'Odoo S.A.','l10','Odoo SA','Camptocamp','Odoo S.A, Applix BV, Droggol Infotech Pvt. Ltd.'] and 'l10' not in x.name and x.name not in ['pf_auto_version_migration', 'report_xlsx'])
        return list
